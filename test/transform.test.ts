...
const nestedWords = [
  { word: "apple", list: "Unit 1::Lesson 1", translation: "n. 苹果" },
  { word: "banana", list: "Unit 1::Lesson 2", translation: "n. 香蕉" },
]
...
describe("transform", () => {
  ...
  it("keeps nested Anki subdeck paths", () => {
    expect(transform(nestedWords, "anki", "My Book")).toBe([
      "#separator:tab",
      "#html:false",
      "#columns:Front Back Deck",
      ["\"apple\"", "\"n. 苹果\"", "\"My Book::Unit 1::Lesson 1\""].join("\t"),
      ["\"banana\"", "\"n. 香蕉\"", "\"My Book::Unit 1::Lesson 2\""].join("\t"),
    ].join("\n"))
  })
})
...
describe("createAnkiApkg", () => {
  it("exports an importable apkg package", async () => {
    ...
    try {
      ...
      expect(Object.values<Record<string, unknown>>(decks).map(deck => deck.name).sort()).toEqual(["My Book", "My Book::Chapter 1", "My Book::Chapter 2"])
  ...
  it("creates parent decks for nested subdeck paths", async () => {
    const apkg = await createAnkiApkg(nestedWords, "My Book")
    const dir = await mkdtemp(join(tmpdir(), "maimemo-apkg-test-"))
    try {
      const apkgPath = join(dir, "deck.apkg")
      await writeFile(apkgPath, apkg)
      ...
      const { stdout: decksJson } = await execFileAsync("sqlite3", [join(dir, "collection.anki2"), "select decks from col"])
      const decks = JSON.parse(decksJson)
      const deckNames = Object.values<Record<string, unknown>>(decks).map(deck => deck.name).sort()
      const deckIds = Object.values<Record<string, unknown>>(decks).map(deck => deck.id)

      expect(deckNames).toEqual([
        "My Book",
        "My Book::Unit 1",
        "My Book::Unit 1::Lesson 1",
        "My Book::Unit 1::Lesson 2",
      ])
